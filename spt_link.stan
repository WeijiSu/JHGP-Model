...
functions {
   //t cdf  
  real  ppt_cdf(real x, real location, real r, real ndf){
    if (r <= 1) return pow(student_t_cdf((x - location) / r,ndf, 0, 1),r) ;
    else  return (1-pow(student_t_cdf(- r * (x - location),ndf,0, 1),(1 / r))) ;
  } 
  // power exponential link
}
...
parameters {
  ...
  real<lower=0> t_df; //df for power t
...
model {
  ...
  for (n in 1:N){
    for(i in 1:T){
      ...
      r[i, n] ~ bernoulli(ppt_cdf(beta0+mu_R[i]+c*(psi[i, n]),0,r_par,t_df));
    }
  }
