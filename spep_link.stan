...
// power exponential link
  ...
  //(double exponential) power laplace cdf  
  real  ppep_cdf(real x,  real r){
    if(r<=1) return( pow(double_exponential_cdf(x/r, 0,1) ,r));
    else return(1-pow(double_exponential_cdf(-x*r, 0,1),1/r));
  }
...
model {
  ...
  for (n in 1:N){
    for(i in 1:T){
      ...
      r[i, n] ~ bernoulli( ppep_cdf(beta0+mu_R[i]+c*(psi[i, n]), r_par));
    }
  }
